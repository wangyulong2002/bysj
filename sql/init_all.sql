-- ============================================================
-- ⚠️ DEPRECATED（P0-4）—— 请勿执行、请勿人工修改
-- ------------------------------------------------------------
-- 本文件是历史遗留的「第二份 DDL 真相」：内含 21 张业务表的手写 CREATE TABLE，
-- 与 Django migrations 并存会导致表结构漂移（"本地能跑、服务器报 Unknown column"）。
--
-- 现已收敛为单一真相（设计 3.3 B-05 / P0-1）：
--   * DDL 权威  = Django migrations（manage/apps/models.py → makemigrations → migrate）
--   * 初始化    = bash scripts/init_db.sh（migrate + sql/seed_dict.sql，不再执行本文件）
--   * 结构导出  = bash scripts/export_ddl.sh --write → sql/schema_export.sql（只读产物）
--
-- 保留本文件仅作历史查阅；如需表结构，请以 sql/schema_export.sql 或 migrations 为准。
-- ============================================================
-- ============================================================
