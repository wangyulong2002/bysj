-- ============================================================
-- seed_dict.sql  字典**数据**初始化（只含 DML，不含任何 DDL）
-- ------------------------------------------------------------
-- 背景（P0-4）：原 sql/init_all.sql 同时承担「建表 DDL」与「字典数据」两件事，
-- 导致 Django migrations 与手写 SQL 形成**两份 DDL 真相**。
-- 现按设计约定收敛：
--   * DDL 权威 = Django migrations（`cd manage && python manage.py migrate`），
--     其中 `sys_dict_type` / `sys_dict_data` 由 `apps/migrations/0002_*` 创建；
--   * 本文件只负责**灌入字典数据**（DML），不含 DROP/CREATE，可安全重复执行。
--
-- 幂等策略：先按 dict_type 删除相关行，再 INSERT，可反复执行。
-- 用法：bash scripts/init_db.sh            # 已集成（migrate 之后自动执行）
--      mysql ... < sql/seed_dict.sql      # 手动执行
-- ============================================================
...
-- ============================================================
-- 清理：仅删除本脚本负责的 dict_type（不触碰其它字典）
-- ============================================================
